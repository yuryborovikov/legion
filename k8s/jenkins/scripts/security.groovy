...
def adminStrategy(GlobalMatrixAuthorizationStrategy strategy, String group) {
...
def readOnlyStrategy(GlobalMatrixAuthorizationStrategy strategy, String group) {
  strategy.add(Jenkins.READ, group)
  strategy.add(hudson.model.Item.READ, group)
  strategy.add(hudson.model.View.READ, group)
  strategy.add(hudson.model.Item.READ, group)
...
if (enableOpenId.toLowerCase() == "true") {
  ...
  String adminGroup = env['OPENID_ADMIN_GROUP']
  String readOnlyGroup = env['OPENID_READ_ONLY_GROUP']
  def strategy = new GlobalMatrixAuthorizationStrategy()

  strategy = readOnlyStrategy(strategy, readOnlyGroup)
  strategy = adminStrategy(strategy, adminGroup)
