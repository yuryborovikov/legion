...
def send_model_id(model_id):
    ...
    :return: None
    """
    send_header_to_stderr(legion_core.headers.MODEL_ID, normalize_name(model_id))
...
def init(model_id=None):
    ...
    if model_id:
        ...
        _model_initialized_from_function = False
        deducted_model_id = os.getenv(*legion_core.config.MODEL_ID)
        if not deducted_model_id:
            raise Exception('Cannot deduct model name. ENV %s is empty' % legion_core.config.MODEL_ID[0])
        else:
