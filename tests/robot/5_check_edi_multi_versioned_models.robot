...
Check EDI deploy 2 models with different versions but the same id
    ...
    Run EDI deploy                      ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_IMAGE_2}
    Should Be Equal As Integers         ${DEPLOY_RESPONSE.rc}         0
    Check model started                 ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}    ${TEST_MODEL_2_VERSION}
    Should contain                      ${MODEL_RESPONSE}       "model_version": "${TEST_MODEL_2_VERSION}"

Check EDI undeploy 1 of 2 models with different versions but the same id
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI undeploy with version   ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}    ${TEST_MODEL_1_VERSION}
    Should Be Equal As Integers     ${UNDEPLOY_RESPONSE.rc}         0
    Run EDI inspect by model id     ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}
    Should Be Equal As Integers     ${INSPECT_RESPONSE.rc}          0
    Should not contain              ${INSPECT_RESPONSE.stdout}      ${TEST_MODEL_IMAGE_1}
    Should contain                  ${INSPECT_RESPONSE.stdout}      ${TEST_MODEL_IMAGE_2}

Check EDI undeploy all model instances by version
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI scale with version      ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}    2   ${TEST_MODEL_1_VERSION}
    Should Be Equal As Integers     ${SCALE_RESPONSE.rc}              0
    Run EDI inspect with parse by model id       ${MODEL_TEST_ENCLAVE}      ${TEST_MODEL_ID}
    ${model_1}=     Find model information in edi   ${PARSED_INSPECT_RESPONSE}      ${TEST_MODEL_ID}   ${TEST_MODEL_1_VERSION}
    Log                             ${model_1}
    ${model_2}=     Find model information in edi   ${PARSED_INSPECT_RESPONSE}      ${TEST_MODEL_ID}   ${TEST_MODEL_2_VERSION}
    Log                             ${model_2}
    ...
    Run EDI inspect by model id     ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}
    Should Be Equal As Integers     ${INSPECT_RESPONSE.rc}              0
    Should not contain              ${INSPECT_RESPONSE.stdout}          ${TEST_MODEL_1_VERSION}
    Should contain                  ${INSPECT_RESPONSE.stdout}          ${TEST_MODEL_2_VERSION}

Check EDI undeploy 1 of 2 models without version
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI undeploy without version   ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}
    Should Be Equal As Integers        ${UNDEPLOY_RESPONSE.rc}         2
    Should contain                     ${UNDEPLOY_RESPONSE.stderr}     Please specify version of model
...
Check EDI scale down 1 of 2 models with different versions but the same id
    ...
    Run EDI scale with version      ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}    1   ${TEST_MODEL_1_VERSION}
    Should Be Equal As Integers     ${SCALE_RESPONSE.rc}              0
    Run EDI inspect with parse by model id       ${MODEL_TEST_ENCLAVE}      ${TEST_MODEL_ID}
    ${model_1}=     Find model information in edi   ${PARSED_INSPECT_RESPONSE}      ${TEST_MODEL_ID}   ${TEST_MODEL_1_VERSION}
    Log                             ${model_1}
...
Check EDI scale up 1 of 2 models by invalid version
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI scale with version      ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}    2   ${TEST_MODEL_1_VERSION}121
...
Check EDI scale up 1 of 2 models without version
    ...
    Should Be Equal As Integers     ${SCALE_RESPONSE.rc}              2
    Should contain                  ${SCALE_RESPONSE.stderr}          Please specify version of model

Check EDI model inspect by model id return 2 models
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model id     ${MODEL_TEST_ENCLAVE}    ${TEST_MODEL_ID}
    Should Be Equal As Integers     ${INSPECT_RESPONSE.rc}          0
    Should contain                  ${INSPECT_RESPONSE.stdout}      ${TEST_MODEL_IMAGE_1}
    Should contain                  ${INSPECT_RESPONSE.stdout}      ${TEST_MODEL_IMAGE_2}

Check EDI model inspect by model version return 1 model
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model version    ${MODEL_TEST_ENCLAVE}    ${TEST_MODEL_1_VERSION}
    Should Be Equal As Integers         ${INSPECT_RESPONSE.rc}          0
    Should contain                      ${INSPECT_RESPONSE.stdout}      ${TEST_MODEL_IMAGE_1}
    Should not contain                  ${INSPECT_RESPONSE.stdout}      ${TEST_MODEL_IMAGE_2}

Check EDI model inspect by model id and version return 1 model
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model id and model version    ${MODEL_TEST_ENCLAVE}   ${TEST_MODEL_ID}      ${TEST_MODEL_1_VERSION}
    Should Be Equal As Integers                      ${INSPECT_RESPONSE.rc}         0
    Should contain                                   ${INSPECT_RESPONSE.stdout}     ${TEST_MODEL_IMAGE_1}
    Should not contain                               ${INSPECT_RESPONSE.stdout}     ${TEST_MODEL_IMAGE_2}
...
Check EDI model inspect by invalid model id
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model id     ${MODEL_TEST_ENCLAVE}    ${TEST_MODEL_ID}test
    Should Be Equal As Integers     ${INSPECT_RESPONSE.rc}          0
    Should be empty                 ${INSPECT_RESPONSE.stdout}

Check EDI model inspect by invalid model version
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model version    ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_1_VERSION}test
    Should Be Equal As Integers         ${INSPECT_RESPONSE.rc}        0
    Should be empty                     ${INSPECT_RESPONSE.stdout}

Check EDI model inspect by invalid model id and invalid version
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model id and model version    ${MODEL_TEST_ENCLAVE}    ${TEST_MODEL_ID}test   ${TEST_MODEL_1_VERSION}test
    Should Be Equal As Integers                      ${INSPECT_RESPONSE.rc}          0
    Should be empty                                  ${INSPECT_RESPONSE.stdout}

Check EDI model inspect by invalid model id and valid version
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model id and model version    ${MODEL_TEST_ENCLAVE}    ${TEST_MODEL_ID}test   ${TEST_MODEL_1_VERSION}
    Should Be Equal As Integers                      ${INSPECT_RESPONSE.rc}          0
    Should be empty                                  ${INSPECT_RESPONSE.stdout}

Check EDI model inspect by valid model id and invalid version
    [Tags]  edi  cli  enclave   multi_versions
    Run EDI inspect by model id and model version    ${MODEL_TEST_ENCLAVE}    ${TEST_MODEL_ID}   ${TEST_MODEL_1_VERSION}test
    Should Be Equal As Integers                      ${INSPECT_RESPONSE.rc}          0
    Should be empty                                  ${INSPECT_RESPONSE.stdout}