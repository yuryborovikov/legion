...
import legion_pyserve.io
import legion_core.model
import pandas
...
def create_simple_summation_model_by_df(path, version):
    ...
    return legion_pyserve.io.export_df(apply,
                                       df,
                                       filename=path,
                                       version=version)
...
def create_simple_summation_model_by_types(path, version):
    ...
    parameters = {
        'a': legion_core.model.int32,
        'b': legion_core.model.int32,
    }

    return legion_pyserve.io.export(apply,
                                    parameters,
                                    filename=path,
                                    version=version)
...
def create_simple_summation_model_untyped(path, version):
    ...
    return legion_pyserve.io.export_untyped(apply,
                                            filename=path,
                                            version=version)
